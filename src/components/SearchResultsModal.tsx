...
export function SearchResultsModal({ query, interpretedQuery, interpretationSource, isInterpreting = false, resultCount, totalCount, semanticStatus, onClose, children }: SearchResultsModalProps) {
  useEffect(() => {
      ...
      <div className="search-results-sheet">
        <header className="search-results-header">
          ...
          <div className="search-results-mode">
            {isInterpreting || semanticStatus === 'searching' ? <LoaderCircle className="animate-spin" /> : <BrainCircuit />}
            <span>{isInterpreting ? 'AI 正在理解搜索意图' : semanticStatus === 'ready' ? '语义索引已参与排序' : semanticStatus === 'searching' ? '正在计算语义相关度' : interpretationSource === 'llm' ? 'AI 已提炼搜索意图' : '内容索引匹配'}</span>
          </div>
